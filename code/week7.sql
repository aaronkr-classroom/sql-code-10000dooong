...
ALTER DATABASE univdb OWNER TO manager; -- postgres 계정으로만 실행 가능(권한부여)

-- 새로운 manager 계정 만들기
SELECT current_user; -- postgres (기본 사용자)

SHOW GRANTS FOR current_user; -- MySQL만
...
-- 테이블에 SELECT 권한 부여
GRANT SELECT ON 학생2 TO manager;
GRANT SELECT ON 과목2 TO manager;
GRANT SELECT ON 수강2 TO manager;
GRANT SELECT ON 과목 TO manager;
GRANT SELECT ON 수강 TO manager;
GRANT SELECT ON 학생2, 과목2, 수강2, 과목, 수강 TO manager;

-- 뷰에 SELECT 권한 부여
GRANT SELECT ON V1_고학년학생 TO manager;
GRANT SELECT ON V2_과목수강현황 TO manager;
GRANT SELECT ON V3_고학년여학생 TO manager;
GRANT SELECT ON V1_고학년학생, V2_과목수강현황, V3_고학년여학생 TO manager;

-- 인덱스
CREATE INDEX idx_수강 ON 수강(학번, 과목번호); -- 오류: 소유자만 가능하다면, postgres 계정에서 실행

CREATE INDEX idx_과목 ON 과목(이름 ASC);
